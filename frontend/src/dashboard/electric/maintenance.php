...
if (!defined('BACKEND_URL')) define('BACKEND_URL', '');
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .custom-scrollbar::-webkit-scrollbar-track { background: #0D0E2A; }
        ...
        .chip.checked .dot { background: #FEBB02; }
        .chip:hover { border-color: rgba(254,187,2,.22); }

        /* ── Stat glows ─────────────────────────────────── */
        .glow-yellow:hover { box-shadow: 0 12px 30px -6px rgba(254,187,2,.18); }
        .glow-green:hover  { box-shadow: 0 12px 30px -6px rgba(95,203,95,.15); }
        .glow-blue:hover   { box-shadow: 0 12px 30px -6px rgba(79,195,247,.15); }
...
══════════════════════════════════════════════════════════ -->
<nav id="sidebar"
     class="flex flex-col fixed lg:sticky top-0 h-screen w-[280px] lg:w-[300px]
    ...
    <div class="flex items-center gap-3 ml-2 mb-10">
        <div class="w-10 h-10 bg-gradient-to-br from-[#FFBB02] to-[#E39A00] rounded-xl flex items-center justify-center shadow-lg shadow-[#FFBB02]/10">
            <svg class="w-6 h-6 text-black" fill="currentColor" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
        </div>
        <div>
            <div class="text-white font-black text-lg tracking-tight leading-tight">POWER<span class="text-[#FFBB02]">GUIDE</span></div>
            <div class="text-white/40 font-bold text-[9px] tracking-widest mt-0.5">ELECTRIC COMPANY</div>
        </div>
    </div>

    <!-- Nav -->
    <div class="flex flex-col gap-1.5">
        <span class="text-[10px] font-black tracking-widest text-white/30 px-3 mb-1 uppercase">Management</span>

        <a href="<?= PUBLIC_URL ?>/dashboard/electric/electric.php"
           class="group flex items-center gap-3.5 px-4 h-11 rounded-xl hover:bg-white/5 hover:text-white transition-all font-semibold text-sm">
            <svg class="w-5 h-5 text-[#B5B5B5] group-hover:text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="flex items-center justify-between gap-3 px-4 py-3 rounded-2xl bg-white/5 border border-white/5">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <div class="text-[10px] font-semibold text-white/40 uppercase tracking-wider">Administrator</div>
                </div>
            </div>
            <a href="<?= BACKEND_URL ?>/public/logout.php"
               class="p-2 text-[#B5B5B5] hover:text-[#CB3435] hover:bg-[#CB3435]/10 rounded-xl transition-all" title="Logout">
...
<main class="flex-1 overflow-y-auto custom-scrollbar flex flex-col w-full">
        ...
        <div>
            ...
            <div class="flex items-center gap-2 mt-1.5">
                <span class="relative flex h-2 w-2">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#FFBB02] opacity-60"></span>
                    <span class="relative inline-flex h-2 w-2 rounded-full bg-[#FFBB02]"></span>
                </span>
                <span class="text-[11px] text-[#B5B5B5] font-semibold tracking-widest uppercase">Location-Mapped System · Auto-Refresh</span>
            </div>
        </div>
    ...
    <div class="p-6 lg:p-10 flex flex-col gap-8">
        ...
        <section class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div class="card-hover glow-yellow bg-[#31324C]/20 border border-white/5 rounded-2xl p-7 flex flex-col gap-4 relative overflow-hidden group">
                <div class="absolute -top-6 -right-6 w-24 h-24 bg-[#FEBB02]/8 rounded-full blur-2xl group-hover:bg-[#FEBB02]/15 transition-all"></div>
                <div class="flex justify-between items-center z-10">
                    ...
                    <span class="text-[#FEBB02] text-[10px] font-black px-2.5 py-1 bg-[#FEBB02]/10 rounded-lg tracking-widest uppercase border border-[#FEBB02]/20">Ongoing</span>
                </div>
                <div class="z-10">
                    <span id="count-ongoing" class="text-white text-5xl font-black tracking-tighter loading-pulse">0</span>
                    <p class="text-[#B5B5B5] text-xs font-semibold mt-1">Active Maintenance</p>
                </div>
            </div>

            <div class="card-hover glow-blue bg-[#31324C]/20 border border-white/5 rounded-2xl p-7 flex flex-col gap-4 relative overflow-hidden group">
                <div class="absolute -top-6 -right-6 w-24 h-24 bg-[#4FC3F7]/8 rounded-full blur-2xl group-hover:bg-[#4FC3F7]/15 transition-all"></div>
                <div class="flex justify-between items-center z-10">
                    ...
                    <span class="text-[#4FC3F7] text-[10px] font-black px-2.5 py-1 bg-[#4FC3F7]/10 rounded-lg tracking-widest uppercase border border-[#4FC3F7]/20">Scheduled</span>
                </div>
                <div class="z-10">
                    <span id="count-scheduled" class="text-white text-5xl font-black tracking-tighter loading-pulse">0</span>
                    <p class="text-[#B5B5B5] text-xs font-semibold mt-1">Upcoming Schedules</p>
                </div>
            </div>

            <div class="card-hover glow-green bg-[#31324C]/20 border border-white/5 rounded-2xl p-7 flex flex-col gap-4 relative overflow-hidden group">
                <div class="absolute -top-6 -right-6 w-24 h-24 bg-[#5FCB5F]/8 rounded-full blur-2xl group-hover:bg-[#5FCB5F]/15 transition-all"></div>
                <div class="flex justify-between items-center z-10">
                    ...
                    <span class="text-[#5FCB5F] text-[10px] font-black px-2.5 py-1 bg-[#5FCB5F]/10 rounded-lg tracking-widest uppercase border border-[#5FCB5F]/20">Completed</span>
                </div>
                <div class="z-10">
                    <span id="count-completed" class="text-white text-5xl font-black tracking-tighter loading-pulse">0</span>
        ...
        <section class="rounded-2xl border border-white/5 overflow-hidden bg-[#31324C]/20 shadow-xl">
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-[#16172E]/50">
                <div class="flex items-center gap-2.5">
                    ...
                    <span class="text-[10px] text-[#B5B5B5] bg-white/5 border border-white/5 px-2 py-0.5 rounded-md font-semibold">Live DB Coordinates</span>
                </div>
                <span class="text-[#FFBB02] px-2.5 py-1 bg-[#FFBB02]/10 text-[10px] rounded-lg font-black border border-[#FFBB02]/20 flex items-center gap-1.5 uppercase tracking-widest">
            ...
            <div class="flex flex-wrap items-center gap-5 px-5 py-3 border-t border-white/5 bg-[#0D0E2A]/60">
                <div class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-[#FEBB02]"></span><span class="text-[10px] text-[#B5B5B5] font-semibold">Ongoing</span></div>
                <div class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-[#4FC3F7]"></span><span class="text-[10px] text-[#B5B5B5] font-semibold">Scheduled</span></div>
                <div class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-[#5FCB5F]"></span><span class="text-[10px] text-[#B5B5B5] font-semibold">Completed</span></div>
            </div>
        </section>
...
<div id="create-modal" class="modal-backdrop" onclick="handleCreateBackdrop(event)">
    <div class="modal-box">
        ...
        <!-- Modal Header -->
        <div class="flex items-center justify-between px-7 py-5 border-b border-white/6 bg-[#16172E]/60 flex-shrink-0">
            <div>
                <h2 class="text-base font-black text-white">New Maintenance Schedule</h2>
                <p class="text-[11px] text-[#B5B5B5] font-medium mt-0.5">Select barangays · coordinates saved to DB</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar px-7 py-6 flex flex-col gap-5">
            ...
            <!-- Company -->
            <div>
                <label class="text-[10px] font-black uppercase tracking-widest text-[#B5B5B5] mb-1.5 block">Company Name</label>
                <input id="f-company" type="text" placeholder="e.g. PowerGuide Pangasinan"
                       class="w-full bg-[#31324C]/30 border border-white/8 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/25
            ...
            <div class="grid grid-cols-3 gap-3">
                <div>
                    <label class="text-[10px] font-black uppercase tracking-widest text-[#B5B5B5] mb-1.5 block">Date</label>
                    <input id="f-date" type="date"
                           class="w-full bg-[#31324C]/30 border border-white/8 rounded-xl px-3 py-2.5 text-sm text-white
                                  focus:outline-none focus:border-[#FEBB02]/50 transition-all [color-scheme:dark]">
                </div>
                <div>
                    <label class="text-[10px] font-black uppercase tracking-widest text-[#B5B5B5] mb-1.5 block">Start Time</label>
                    <input id="f-start" type="time"
                           class="w-full bg-[#31324C]/30 border border-white/8 rounded-xl px-3 py-2.5 text-sm text-white
                                  focus:outline-none focus:border-[#FEBB02]/50 transition-all [color-scheme:dark]">
                </div>
                <div>
                    <label class="text-[10px] font-black uppercase tracking-widest text-[#B5B5B5] mb-1.5 block">End Time</label>
            ...
            <!-- Radius slider -->
            <div>
                <label class="text-[10px] font-black uppercase tracking-widest text-[#B5B5B5] mb-1.5 flex items-center gap-1">
                    Coverage Radius
            ...
            <!-- Description -->
            <div>
                <label class="text-[10px] font-black uppercase tracking-widest text-[#B5B5B5] mb-1.5 block">Description <span class="text-white/20 normal-case font-medium">(optional)</span></label>
                <textarea id="f-desc" rows="2" placeholder="Brief description of the maintenance work…"
                          class="w-full bg-[#31324C]/30 border border-white/8 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/25
            ...
            <div>
                <div class="flex items-center justify-between mb-2">
                    <label class="text-[10px] font-black uppercase tracking-widest text-[#B5B5B5]">Affected Barangays</label>
                    <div class="flex gap-1.5">
                        <button onclick="selectAll()" class="text-[10px] font-bold px-2.5 py-1 bg-[#FEBB02]/10 text-[#FEBB02] border border-[#FEBB02]/20 rounded-lg hover:bg-[#FEBB02]/20 transition-all">All</button>
                        <button onclick="clearAll()" class="text-[10px] font-bold px-2.5 py-1 bg-white/5 text-[#B5B5B5] border border-white/8 rounded-lg hover:bg-white/10 transition-all">Clear</button>
                    </div>
                </div>
            ...
            <!-- Error -->
            <div id="create-error" class="hidden text-xs text-[#CB3435] bg-[#CB3435]/8 border border-[#CB3435]/20 rounded-xl px-4 py-3 font-semibold"></div>

        </div>

        <!-- Modal Footer -->
        <div class="flex items-center justify-end gap-3 px-7 py-4 border-t border-white/5 bg-[#0D0E2A]/80 flex-shrink-0">
            <button onclick="closeCreateModal()"
...
<div id="view-modal" class="modal-backdrop" onclick="if(event.target===this)closeViewModal()">
    <div class="modal-box">
        <div class="flex items-center justify-between px-7 py-5 border-b border-white/6 bg-[#16172E]/60 flex-shrink-0">
            ...
            <button onclick="closeViewModal()" class="text-[#B5B5B5] hover:text-white hover:bg-white/8 p-2 rounded-xl transition-all">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        ...
        <div class="flex justify-end gap-3 px-7 py-4 border-t border-white/5 bg-[#0D0E2A]/80 flex-shrink-0">
            <button onclick="closeViewModal()" class="px-5 py-2.5 rounded-xl text-sm font-bold text-[#B5B5B5] hover:text-white border border-white/8 hover:bg-white/5 transition-all">Close</button>
...
<div id="delete-modal" class="modal-backdrop" onclick="if(event.target===this)closeDeleteModal()">
    <div class="modal-box modal-box-sm text-center">
        <div class="px-7 py-8 flex flex-col items-center gap-5">
            <div class="w-16 h-16 bg-[#CB3435]/10 border border-[#CB3435]/20 rounded-2xl flex items-center justify-center">
                <svg class="w-8 h-8 text-[#CB3435]" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                </svg>
            </div>
            <div>
                <h3 class="text-white font-black text-lg">Delete Schedule?</h3>
                <p class="text-[#B5B5B5] text-sm mt-1.5 leading-relaxed">This will permanently delete the maintenance record and all linked barangay location data.</p>
            </div>
